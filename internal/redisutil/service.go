...
func NewRedisService(logger *slog.Logger, redisClient *redis.Client) (*RedisService, error) {
	ctx := context.Background()
	pong, err := redisClient.Ping(ctx).Result()
	if err != nil {
		return nil, err
	}
	serviceLogger := logger.With("service", "redis")
	serviceLogger.Info("Connected to Redis", "ping", pong)
...
// AddToBlocklist adds a token JTI (JWT ID) to the Redis blocklist with an expiration.
// The expiration should ideally match the original token's remaining lifetime.
func (s *RedisService) AddToBlocklist(ctx context.Context, jti string, expiresAt time.Time) error {
	now := time.Now()
	if expiresAt.Before(now) || expiresAt.Equal(now) {
		s.logger.Debug("Token already expired, not adding to blocklist", "jti", jti)
		return nil // Already expired
	}
	duration := expiresAt.Sub(now)
	err := s.Client.Set(ctx, "bl:"+jti, "revoked", duration).Err()
	if err != nil {
		s.logger.Error("Failed to add token JTI to Redis blocklist", "jti", jti, "error", err)
	} else {
		s.logger.Info("Token JTI added to blocklist", "jti", jti, "duration", duration)
	}
	return err
}

// IsBlocklisted checks if a token JTI is in the Redis blocklist.
func (s *RedisService) IsBlocklisted(ctx context.Context, jti string) (bool, error) {
	val, err := s.Client.Exists(ctx, "bl:"+jti).Result()
	if err != nil {
		s.logger.Error("Failed to check token JTI in Redis blocklist", "jti", jti, "error", err)
		return false, err // Fail secure: assume blocklisted on error, or handle error differently
	}
	return val == 1, nil
}
